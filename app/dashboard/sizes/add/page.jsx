...
export default function AddSizePage() {
    const [formData, setFormData] = useState({ name: "", status: "active", centralStatus: "active" });
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        setIsLoading(true);
        setTimeout(() => { alert("Size saved!"); setIsLoading(false); }, 1000);
    };
        ...
        <div className="max-w-2xl mx-auto text-black">
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-4">
                    <Link href="/dashboard/sizes" className="text-black hover:opacity-70 transition-opacity cursor-pointer">
                        <ArrowLeft className="h-5 w-5" />
                    </Link>
                    <h1 className="text-2xl font-bold text-black">Add Size</h1>
                </div>
                <button onClick={handleSubmit} disabled={isLoading} className="flex items-center gap-2 bg-black text-white px-5 py-2.5 hover:bg-gray-800 transition-colors text-sm font-bold border border-black cursor-pointer">
                    <Save className="h-4 w-4" /> Save Size
                </button>
            </div>
            ...
            <form className="space-y-6">
                <div className="bg-white p-6 border border-black">
                    <div className="grid grid-cols-1 gap-6">
                        <div>
                            <label className="block text-sm font-bold mb-1">Size Name (e.g. Small, XL, 42)</label>
