...
import { Plus, Search, Edit, Trash2, Coins, MoreHorizontal } from "lucide-react";

export default function CurrenciesPage() {
    // Dummy data representing your Currency schema
    const [currencies] = useState([
        { _id: "cur_1", name: "US Dollar", cc: "USD", symbol: "$", centralStatus: "active" },
        { _id: "cur_2", name: "Euro", cc: "EUR", symbol: "€", centralStatus: "active" },
        { _id: "cur_3", name: "British Pound", cc: "GBP", symbol: "£", centralStatus: "inactive" },
    ]);
    ...
    return (
        <div className="w-full max-w-7xl mx-auto space-y-6 pb-12 text-slate-900">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold tracking-tight text-slate-900">Platform Currencies</h1>
                    <p className="text-sm font-medium mt-1 text-slate-500">Manage global currencies supported across the platform.</p>
                </div>
                <Link
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.08)] overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full min-w-[760px] text-left text-sm whitespace-nowrap">
                        <thead className="bg-slate-50 border-b border-slate-100">
                            <tr>
                                <th className="px-6 py-4 font-semibold text-slate-500 text-xs uppercase tracking-wider">Currency</th>
                                <th className="px-6 py-4 font-semibold text-slate-500 text-xs uppercase tracking-wider">ISO Code (CC)</th>
                                <th className="px-6 py-4 font-semibold text-slate-500 text-xs uppercase tracking-wider">Symbol</th>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {currencies.map((currency) => (
                                <tr key={currency._id} className="hover:bg-slate-50/80 transition-colors">
                                    ...
                                    <td className="px-6 py-4 font-mono font-bold text-slate-700">{currency.cc}</td>
                                    <td className="px-6 py-4 font-bold text-slate-900 text-lg">{currency.symbol}</td>
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider border ${
                                            currency.centralStatus === 'active' 
                                            ? 'bg-indigo-50 text-indigo-700 border-indigo-100' 
                                            : 'bg-red-50 text-red-700 border-red-100'
                                        }`}>
                                            {currency.centralStatus}
                                        </span>
                                    </td>
                ...
                <div className="divide-y divide-slate-100 md:hidden bg-white">
                    {currencies.map((currency) => (
                        <article key={currency._id} className="p-5 flex items-center justify-between">
                            <div>
                                <h2 className="text-sm font-bold text-slate-900">{currency.name}</h2>
                                <p className="text-xs font-mono text-slate-500 mt-0.5">{currency.cc} ({currency.symbol})</p>
                            </div>
                            <div className="flex items-center gap-3">
                                <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase border ${currency.centralStatus === 'active' ? 'bg-indigo-50 text-indigo-700 border-indigo-100' : 'bg-red-50 text-red-700 border-red-100'}`}>
                                    {currency.centralStatus}
                                </span>
