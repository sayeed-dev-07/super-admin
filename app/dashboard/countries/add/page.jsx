...
import { Save, ArrowLeft, Globe, ShieldAlert } from "lucide-react";
import Link from "next/link";

export default function AddCountryPage() {
    const [isLoading, setIsLoading] = useState(false);

    // Form state matching your Country schema
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            console.log("Sending country payload:", formData);
            // Simulate API call
        ...
        <div className="max-w-4xl mx-auto space-y-6 pb-20 text-slate-900">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 sticky top-0 bg-slate-50/80 backdrop-blur-md z-20 py-4 border-b border-slate-200 -mx-4 px-4 sm:mx-0 sm:px-0">
                <div className="flex items-center gap-3">
                    <Link href="/dashboard/countries" className="p-2 rounded-xl bg-white border border-slate-200 text-slate-400 hover:text-[#5B45FF] transition-colors shadow-sm cursor-pointer">
                        <ArrowLeft className="h-5 w-5" />
                    </Link>
                    <div>
                        <h1 className="text-xl font-bold text-slate-900">Add Country</h1>
                        <p className="text-xs font-medium text-slate-500 mt-0.5">Register a new region available to shops.</p>
                    </div>
                </div>
                <button
                ...
                >
                    ...
                    {isLoading ? "Processing..." : "Save Country"}
                </button>
            </div>
            ...
            <form className="space-y-8 mt-6">
                ...
                <div className="bg-white p-6 md:p-8 rounded-2xl border border-slate-100 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.05)]">
                    <h3 className="text-sm font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2 mb-6 border-b border-slate-100 pb-3">
                        <Globe className="h-4 w-4" /> Regional Details
                    </h3>
                ...
                <div className="bg-white p-6 md:p-8 rounded-2xl border border-slate-100 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.05)]">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <Select 
                            ...
                            onChange={handleChange} 
                            options={[{v:'active', l:'Active (Available)'}, {v:'inactive', l:'Inactive (Disabled)'}]} 
                        />
                        <div>
